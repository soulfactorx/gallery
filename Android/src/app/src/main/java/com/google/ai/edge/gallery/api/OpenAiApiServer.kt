    ...
    private suspend fun handleChatCompletions(call: ApplicationCall) {
        ...
        try {
            engine.createConversation(
                ...
                // system 메시지를 첫 user 메시지에 prefix로 붙이는 방식
                ...
                for (i in userMessages.indices) {
                    ...
                    if (i < userMessages.size - 1) {
                        // 히스토리 메시지 (마지막 제외) → 그냥 전송해서 상태만 쌓기
                        if (msg.role == "user") {
                            val content = if (i == 0 && systemMsg != null)
                                "$systemMsg\n\n${msg.content}" else msg.content
                            conversation.sendMessageAsync(content).collect { /* discard */ }
                        }
                        // assistant 메시지는 LiteRT-LM이 자체적으로 히스토리 관리
                    } else {
